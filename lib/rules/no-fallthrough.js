...
/**
 * Checks whether a node has a trailing comment in a new line
 * @param {ASTNode} node - The node to check
 * @returns {boolean} `true` if the node has a traling comment in a new line
 */
function hasTrailingCommentsInNewLine(node) {
    var lastComment = lodash.last(node.trailingComments);

    return lastComment && lastComment.loc.start.line > node.loc.end.line;
}
    ...
    return {
        ...
        "SwitchCase:exit": function(node) {
            ...
            if (currentCodePath.currentSegments.some(isReachable) &&
                (node.consequent.length > 0 || hasTrailingCommentsInNewLine(node)) &&
